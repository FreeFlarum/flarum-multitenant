    ...
    fetch(url)
        ...
        .then((content) => {
            ...
            for (var i = 0; i < parseInt(giphyLimit); i += 2) {
                ...
                imgL.style =
                    'min-width: 97.5%; width: 97.5%; border-radius: 5px; margin: 1.25%; margin-left: 0%; margin-right: 0.3125%; vertical-align: top; cursor: pointer;';
                ...
                imgR.style =
                    'min-width: 97.5%; width: 97.5%; border-radius: 5px; margin: 1.25%; margin-left: 0.3125%; margin-right: 0%; vertical-align: top; cursor: pointer;';
...
export default class SearchGIFModal extends Modal {
    ...
    content() {
        return m(
            ...
            m('div', [
                m('table[style = vertical-align: top; width: 100%;]', [
                    m('td[style = width: 100%;]', [
                        m(
                            'form[style = margin-right: 1.25%;]',
                            {
                                class: 'Search-input',
                                onsubmit: () => {
                                    return false;
                                ...
                                onkeyup: (e) => {
                                    if (e.keyCode === 13) {
                                        ...
                                        document.getElementsByClassName('temp-text')[0].textContent = app.translator.trans(
                                            'therealsujitk.forum.gifs.loading'
                                        );
                                        document.getElementById('LoadMore').style.visibility = 'hidden';
                                        const textarea = this.attrs.textArea;
                                        const giphyAPI = app.forum.attribute('therealsujitk-gifs.giphy_api_key');
            ...
            m('div[style = margin-top: 10px; margin-bottom: 10px; min-height: 45vh; height: 45vh; overflow: auto;]', [
                ...
                m(
                    'button[style = position: relative; left: 50%; transform: translate(-50%, 0%); margin-top: 10px; visibility: hidden;]',
                    {
                        id: 'LoadMore',
                        class: 'Button',
                        type: 'button',
            ...
            m('div[style = padding-top: 10px; padding-bottom: 30px;]', [
                m('img[style = float: right;]', {
